...
void AndroidEngine::play()
{
    ...
    if (streamState != oboe::StreamState(3) && streamState != oboe::StreamState(4))
    {
...
void AndroidEngine::onErrorBeforeClose(oboe::AudioStream *audioStream, oboe::Result error)
{
    LOGE("Audio stream error before close: %s", oboe::convertToText(error));
}

void AndroidEngine::onErrorAfterClose(oboe::AudioStream *audioStream, oboe::Result error)
{
...
void AndroidEngine::recreateStream()
{
    oboe::AudioStreamBuilder builder;
    builder.setSharingMode(oboe::SharingMode::Shared)
        ->setPerformanceMode(oboe::PerformanceMode::LowLatency)
